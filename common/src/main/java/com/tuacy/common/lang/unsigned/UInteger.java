...
public final class UInteger extends UNumber implements Comparable<UInteger> {
     ...
     *                               of an <code>unsigned byte</code>
     */
    public static UInteger valueOf(UShort low, UShort high) throws NumberFormatException {
        return valueOfUnchecked((high.intValue() << 16 | low.intValue()) & MAX_VALUE);
    }

    /**
     * Create an <code>unsigned int</code> by byte array of length 4
     * @param bytes
     * @return
     * @throws NumberFormatException
     */
    public static UInteger valueOf(byte[] bytes) throws NumberFormatException {
        if (bytes == null || bytes.length != 4) {
            throw new NumberFormatException("byte长度错误，无法转换");
        }

        return valueOfUnchecked(((bytes[0] & 0xFF) | (bytes[1] & 0xFF) << 8 | (bytes[2] & 0xFF) << 16 | (bytes[3] & 0xFF) << 24) & MAX_VALUE);
    }
